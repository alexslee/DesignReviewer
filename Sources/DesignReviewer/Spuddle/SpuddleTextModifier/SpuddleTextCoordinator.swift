...
class SpuddleTextCoordinator: DesignReviewCoordinatorProtocol {
  ...
  func start() {
    let spuddle = Spuddle(
      ...
      view: { [weak self] in
        if let self = self {
          SpuddleTextModifierView(viewModel: self.textViewModel)
        } else {
          EmptyView()
        }
      }, backgroundView: {
    ...
    textViewModel.dismissHandler = { [weak self] in
      self?.dismissHandler()
    }
